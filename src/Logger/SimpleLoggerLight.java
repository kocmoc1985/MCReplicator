        ...
        try {
            ...
            if (out != null) {
                try {
                    ...
                    Logger.getLogger(SimpleLoggerLight.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        }
    }
...
//    }
    private static String build_log_message(String type, String text_to_write) {
        if (type.equals(DEFAULT_LOG_MSG)) {
            return "[" + HelpM.get_proper_date_time_same_format_on_all_computers() + "] " + "  " + text_to_write;
        } else if (type.equals(EXCEPTION_LOG_MSG)) {
            return "[" + HelpM.get_proper_date_time_same_format_on_all_computers() + "] "
